...
# 经向样本 69;  块状疵点  162;   纬向疵点  219;    正常疵点  300     训练样本  750
# 经向样本 221 ； 块状疵点 205；  纬向疵点 214； 正常疵点 205  测试样本一共    845 
# 训练样本和测试样本可以颠倒试验
...
# 加载训练数据集
def load_train_data():
#    trainfile = r"D:\task\12月\机器学习\128-128实验样本\No12\训练样本"  # 750 做训练
    trainfile = r"D:\task\12月\机器学习\128-128实验样本\No12\测试样本"  # 845  做训练
...
#    train_data = np.zeros((750,128*128))   # 0矩阵 (789,128*128)
#    train_data_number = np.zeros(750)      # 训练数据的样本个数
    
    train_data = np.zeros((845,128*128))   # 0矩阵 (1342,128*128)
    train_data_number = np.zeros(845)      # 训练数据的样本个数
    index  = 0
    differ = 0      # 分类  共分为4类样本
    for file in dirs:
        differ += 1
...
# 加载测试数据集
def load_test_data():
#    testfile = r"D:\task\12月\机器学习\128-128实验样本\No12\测试样本"  # 845 做测试
    testfile = r"D:\task\12月\机器学习\128-128实验样本\No12\训练样本"  # 750  做测试
...
#    test_data = np.zeros((845,128*128))
#    test_data_number = np.zeros(845)
    
    test_data = np.zeros((750,128*128))
    test_data_number = np.zeros(750)
    index  = 0
    differ = 0
    for file in dirs:
        differ += 1
...
def load_set():
    # 750 训练  845 测试
#    train_data,train_data_number = load_train_data()  # (750,16384)  (750,)
#    test_data,test_data_number = load_test_data()     #(845,16384)   (845,)
    # 845 训练  750 测试
    train_data,train_data_number = load_train_data()  # (845,16384)  (845,)
    test_data,test_data_number = load_test_data()     #(750,16384)   (750,)
...
def average():        
    ...
    return sum/20
# No12 文件夹
# 750 张训练  845 张测试 正确率 大约 59.93%
# 845 张训练  750 张测试  正确率 大约 75.13%
